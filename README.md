...
pyhton kchart.py
```
即可生成K线图，所有参数都有默认值，可直接运行，想生成你指定的K线图，可以使用如下参数
```

-h     显示帮助信息
-tp    交易对，默认：BTCUSDT
-p     K线周期，默认：1m
-s     开始时间，格式：YYYY-MM-DD HH:MM
-e     结束时间，格式：YYYY-MM-DD HH:MM
-n     获取K线数量
```
...
示例：
```python
python kchart.py -tp BTCUSDT -p 1m -s "2025-01-01 00:00" -e "2025-01-01 01:00"
```
...
示例：
```python
python kchart.py -tp ETHUSDT -p 5m -s "2025-01-01 00:00" -n 100
```
...
示例：
```python
python kchart.py -tp BTCUSDT -p 1h -e "2025-01-01 12:00" -n 200
```
...
python kchart.py
```
效果：BTCUSDT 1m周期，最新100根K线

2. 只修改交易对和周期
```python
python kchart.py -tp ETHUSDT -p 1h
```

3. 只修改K线数量
```python
python kchart.py -n 200
```
